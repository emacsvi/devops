...
# 放到fstab里面，开机自启动
echo "/dev/sda1 /data/filecoin ext4 rw,noatime 0 0" | tee -a /etc/fstab
```
...
sudo apt update
sudo apt-get -o Acquire::http::proxy="http://182.140.213.155:10809/" install golang-go -y
sudo apt install  gcc git bzr jq pkg-config mesa-opencl-icd ocl-icd-opencl-dev -y
# 设代理
export {http,https}_proxy='http://182.140.213.155:10809'
# 安装rust
